     ...
     var _TOCNode = declare([_Widget, _Templated], {
         ...
         postCreate: function () {
             ...
             if (this.legend) {
                 ...
                 this._createServiceLayerNode(this.serviceLayer, this.rootLayer);
         ...
         _createRootLayerNode: function (rootLayer) {
             ...
             if (!this.rootLayerTOC.config.noSubLayers) { // JAG
                 ...
                 // no sublayers means no need for plus/minus sign /// JAG
                 domStyle.set(this.iconNode, 'visibility', 'hidden');  // JAG
             } // JAG
         ...
         _createServiceLayerNode: function (serviceLayer, rootLayer) {
             ...
             if (serviceLayer._subLayerInfos) {// group layer
                 ...
                 this._noCheckNode = true; // JAG remove the checkbox from all group layers - expand only
                 this._createChildrenNodes(serviceLayer._subLayerInfos, 'serviceLayer');
             } else {
                 if (rootLayer.url) { // JAG add a hyperlink to show service information in a new window
                     var link = rootLayer.url + "/" + serviceLayer.id;
                     this.labelNode.innerHTML = "<a href='" + link + "' target='_blank'>" + serviceLayer.name + "</a>";
                 }
                 else {
                     this.labelNode.innerHTML = serviceLayer.name;
                 }
         ...
         _onClick: function (evt) {
             ...
             if (t == this.checkNode || dijit.getEnclosingWidget(t) == this.checkNode) {
                 ...
                 if (this.serviceLayer) {
                     ...
                     if (this.serviceLayer._subLayerInfos) {
                         ...
                         // 2013-08-01 handler multiple level of groups
                         // JAG -- this line makes checking the group layer TURN-ON all subLayers
                         // JAG -- COMMENTED THIS OUT 
             ...
             } else if (t == this.iconNode || t == this.labelNode) { // JAG enable labelNode to toggleContainer TOO
